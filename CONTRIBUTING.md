...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo BlusceLabs/hermes-agent "<your terms>"
  gh search prs --repo BlusceLabs/hermes-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/BlusceLabs/hermes-agent/issues?q=) · [PRs (all states)](https://github.com/BlusceLabs/hermes-agent/pulls?q=is%3Apr).
...
├── tests/                    # Test suite
├── website/                  # Documentation site (bluscelabs.github.io/hermes-agent)
│

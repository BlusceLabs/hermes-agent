...
- **Profile 安全路径**：不得硬编码 `~/.hermes` — 代码路径使用 `hermes_constants` 中的 `get_hermes_home()`，面向用户的消息使用 `display_hermes_home()`。完整规则参见 [AGENTS.md](https://github.com/BlusceLabs/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/BlusceLabs/hermes-agent/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/BlusceLabs/hermes-agent/blob/main/LICENSE) 授权。
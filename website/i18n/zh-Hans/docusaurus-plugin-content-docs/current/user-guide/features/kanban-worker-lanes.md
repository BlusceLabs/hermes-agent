...
如果你编写的是 worker 代码本身——即运行在通道*内部*的 agent——kanban 生命周期与参考细节会自动注入到 worker 的系统提示中（[`agent/prompt_builder.py`](https://github.com/BlusceLabs/hermes-agent/blob/main/agent/prompt_builder.py) 中的 `KANBAN_GUIDANCE` 块）。
...
相关历史 issue 为 [#19931](https://github.com/BlusceLabs/hermes-agent/issues/19931)，以及已关闭未合并的 Codex 专项 PR [#19924](https://github.com/BlusceLabs/hermes-agent/pull/19924)——这些描述了原始架构提案，但未落地运行器。
...
- [`KANBAN_GUIDANCE`](https://github.com/BlusceLabs/hermes-agent/blob/main/agent/prompt_builder.py) — 注入到每个 kanban worker 系统提示中的 worker + orchestrator 生命周期。
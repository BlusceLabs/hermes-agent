...
```bash
/cron add "every 6h" "Check the GitHub repository BlusceLabs/hermes-agent for:
- New issues opened in the last 6 hours
- New PRs opened or merged in the last 6 hours
- Any new releases

Use the terminal to run gh commands:
  gh issue list --repo BlusceLabs/hermes-agent --state open --json number,title,author,createdAt --limit 10
  gh pr list --repo BlusceLabs/hermes-agent --state all --json number,title,author,createdAt,mergedAt --limit 10

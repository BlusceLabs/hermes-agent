...
To easily install the command-line and desktop applications, [download the Hermes Desktop installer](https://bluscelabs.github.io/hermes-agent/) from our website and run it.
...
2. **As the unprivileged service user**, run the regular installer. It will detect the missing sudo, skip `--with-deps`, and install Chromium into the user's local Playwright cache:
   ...
   ```bash
   curl -fsSL https://bluscelabs.github.io/hermes-agent/scripts/install.sh | bash -s -- --skip-browser
   ```

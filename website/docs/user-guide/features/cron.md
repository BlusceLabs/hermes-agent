...
Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/BlusceLabs/hermes-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
